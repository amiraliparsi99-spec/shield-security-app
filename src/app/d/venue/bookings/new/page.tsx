...
export default function NewBookingPage() {
  ...
  >([]);
  const [locationSource, setLocationSource] = useState<LocationSource>("profile");
  ...
  const [saveNewSiteToLibrary, setSaveNewSiteToLibrary] = useState(true);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .select("id, label, address_line1, city, postcode, country_code, latitude, longitude")
        .eq("venue_id", venue.id)
        .order("label");
      if (!cancelled && !error && data) setSavedLocations(data);
    })();
    return () => {
      cancelled = true;
    };
        ...
        <div className="glass rounded-xl p-6">
          ...
          {step === 2 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-400 mb-1">Required Attire</label>
                <select
