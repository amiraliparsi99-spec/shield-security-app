...
function CheckoutForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: submitError } = await stripe.confirmPayment({
      elements,
      confirmParams: {
        return_url: `${window.location.origin}${basePath}/bookings/${booking.id}/pay?success=true`,
      },
    });
...
export function BookingPayment({ bookingId, basePath }: BookingPaymentProps) {
  ...
  const initiatePayment = async () => {
    ...
    const totalPence =
      Math.round(booking.estimated_total * 100) ||
  ...
  const platformFee = Math.round(totalPence * 0.1);
    ...
    <div className="max-w-lg mx-auto p-6 pt-8">
      ...
      {clientSecret ? (
        ...
        >
          ...
          >
            <CheckoutForm booking={booking} amount={totalPence} basePath={basePath} />
          </Elements>
        </motion.div>
      ) : (
        <motion.div
