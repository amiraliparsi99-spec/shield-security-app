/** Minimum remaining time before we offer "find cover" (agency prompt). */
export const MIN_REMAINING_MINUTES_FOR_COVER = Number(
  process.env.COVER_MIN_REMAINING_MINUTES ?? 60,
);

/** Guards can claim urgent in-progress cover down to this many minutes left. */
export const MIN_REMAINING_MINUTES_TO_CLAIM_URGENT = Number(
  process.env.COVER_MIN_CLAIM_MINUTES ?? 15,
);
...
export function isShiftInProgress(
  shift: Pick<MarketplaceShift, "scheduled_start" | "scheduled_end">,
  nowMs: number = Date.now(),
): boolean {
  return (
    hasShiftStarted(shift.scheduled_start, nowMs) &&
    remainingMinutes(shift.scheduled_end, nowMs) > 0
  );
}
...
export function coverEligibleRemainingMinutes(
  scheduledEnd: string,
  nowMs: number = Date.now(),
): number {
  return remainingMinutes(scheduledEnd, nowMs);
}

export function canAgencyOfferCover(
  scheduledEnd: string,
  nowMs: number = Date.now(),
  force?: boolean,
): boolean {
  if (force) return remainingMinutes(scheduledEnd, nowMs) > 0;
  return (
    remainingMinutes(scheduledEnd, nowMs) >= MIN_REMAINING_MINUTES_FOR_COVER
  );
}
