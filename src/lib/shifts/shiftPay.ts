...
export function isEarlyEnd(
  ...
  const actualMs = new Date(shift.actual_end).getTime();
  const schedMs = new Date(shift.scheduled_end).getTime();
  return actualMs < schedMs - bufferMs;
}
...
export function getShiftCompletionDisplay(
...
): { label: string; detail: string | null; tone: "success" | "warning" | "muted" | "danger" } {
  ...
  switch (kind) {
    ...
    case "in_progress":
      return { label: "On shift", detail: payLine, tone: "success" };
    case "cancelled":
      return {
        label: "Cancelled",
    ...
    case "no_show":
      return { label: "No show", detail: null, tone: "danger" };
    default:
      return { label: "Scheduled", detail: null, tone: "muted" };
  }
}

export function workPayStatusText(opts: {
  status?: string;
...
export function paymentStatusLabel(opts: {
  ...
  self_managed?: boolean | null;
}): "paid" | "pending" | "processing" | "payroll" | "none" {
  if (opts.escrow_status === "released" || opts.escrow_status === "paid") {
    return "paid";
  }
  if (shiftHasRecordedWork({ status: opts.status ?? "" })) {
    if (opts.self_managed) return "payroll";
    return opts.venue_confirmed ? "processing" : "pending";
  }
  return "none";
}
