-- Align notify_guards_for_booking SQL fallback with normalized availability + blocked dates
-- (and basic personnel preferences). Mirrors app logic in notify-guards.ts where possible.
...
BEGIN
  ...
  v_local_ts   := v_shift.scheduled_start AT TIME ZONE 'Europe/London';
  v_local_date := v_local_ts::date;
  v_local_dow  := EXTRACT(DOW FROM v_local_ts)::integer;
  v_local_hour := EXTRACT(HOUR FROM v_local_ts)::integer;
  v_is_night   := (v_local_hour >= 22 OR v_local_hour < 6);
  ...
  FOR v_guard IN
    ...
    WHERE p.is_active = true
      ...
      AND (
        COALESCE(p.weekend_only, false) = false
        OR v_local_dow IN (0, 6)
      )
      AND (
        COALESCE(p.night_shifts_ok, true) = true
        OR NOT v_is_night
      )
